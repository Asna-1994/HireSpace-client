...
const JobDetailsModal: React.FC<JobDetailsModalProps> = ({ jobPost, isOpen, onClose }) => {
...
const { company, isAuthenticated } = useSelector((state: RootState) => state.auth);
    ...
    const handleEdit = () => {
        navigate(`/company/${company?._id}/create-job-posts`, { state: { jobPost } });
      };
      

      const handleDelete = async() => {
        try{
const response  = await axiosInstance.delete(`/company/job-post/${jobPost._id}`)
if(response.data.success){
  ...
  return (
    <div className="fixed  inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
     ...
     <button onClick={handleEdit} className="bg-green-600 text-white px-3 py-1 rounded-lg ">
            Edit
          </button>
          <button onClick={handleDelete} className="bg-red-800 text-white px-3 py-1 rounded-lg ">
           Delete
          </button>
     </div>
   ...
   <button onClick={onClose} className="bg-blue-600 text-white px-4 py-2 rounded-lg mt-4">
          Close
        </button>
