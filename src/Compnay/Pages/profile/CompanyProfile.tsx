...
const getCompanyProfileDetails = async() => {
  try{
    const response = await axiosInstance.get(`/company/company-profile-details/${company?._id}`)
...
{company?.companyLogo?.url ? (
      ...
      <div className="mx-auto mb-4 w-24 h-24 rounded-full shadow-lg bg-gray-500 flex items-center justify-center text-4xl font-bold">
        {getCompanyInitials(company?.companyName as string)}
            ...
            <h1 className="text-5xl font-extrabold mb-2">{company?.companyName}</h1>
            ...
            <p className="mt-2 text-lg flex items-center justify-center">
              ...
              Trusted in <span className="font-semibold ml-1">{company?.industry}</span> industry
            </p>
            <p className="mt-2 text-lg flex items-center justify-center">
            ...
            <p className="mt-2 text-lg flex items-center justify-center">
              ...
              Established: {company?.establishedDate ? new Date(company.establishedDate).toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
  }) : "Unknown"}
        ...
        <section className="py-12 px-6">
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { title: "Mission", content: additionalDetails.mission, icon: <FaGlobe /> },
              { title: "Vision", content: additionalDetails.vision, icon: <FaCrown /> },
              { title: "Phone", content: company?.phone, icon: <FaPhone /> },
              { title: "CEO", content: additionalDetails.ceo, icon: <FaCrown /> },
              { title: "Website", content: additionalDetails.website, icon: <FaGlobe /> },
              { title: "Registration Number", content: company?.documentNumber, icon: <FaCertificate /> },
              { title: "App Plan", content: company?.appPlan, icon: <FaGlobe /> },
            ].map((item, index) => (
              <div
                key={index}
