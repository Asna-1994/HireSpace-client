...
import { addOrUpdate, deleteEducation, fetchEducation } from "../../services/user/userProfileService";
...
const useEducation = (userId: string | undefined) => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        yearOfPassing: Yup.string().matches(/^\d{4}$/, 'Enter a valid 4-digit year').required('Required'),
    ...
    const addOrUpdateEducation = async (education: Education, educationId?: string) => {
        if (!userId) return;

        try {
            const data = await addOrUpdate(education, userId, educationId);
            if (data.success) {
                toast.success(educationId ? 'Education updated successfully' : 'Education added successfully');
                getEducations();
            } else {
