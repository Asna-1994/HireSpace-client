...
const AllConnections = () => {
  ...
  const handleSendRequestForConnection = async (sender: string, receiver: string) => {
    try {
      const response = await axiosInstance.post(`/connection-request`, { fromUser: sender, toUser: receiver });
      if (response.data.success) {
...
useEffect(() => {
    ...
    <>
      ...
      <section className="bg-gray-50 py-16 min-h-screen">
        <div className="container mx-auto px-6">
          <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">People You May Know</h1>
            ...
            <div className="col-span-2">
              <div className="grid grid-cols-1 gap-8">
                {recommendations.length > 0 ? (
                  ...
                  .map((recommendation) => (
                    <div key={recommendation._id} className="bg-white shadow-md rounded-lg p-6">
                      <div className="flex items-center">
                        ...
                        <div>
                          <h3 className="text-gray-800 font-semibold">{recommendation.userName}</h3>
                          <p className="text-sm text-gray-500">{recommendation.email}</p>
                        </div>
                        <button
                          onClick={() => handleSendRequestForConnection(user?._id as string, recommendation._id)}
                          className="bg-blue-500 text-white px-3 py-1 rounded-full text-sm hover:bg-blue-600 transition duration-300 ml-auto"
                        >
                          Connect
                        </button>
                      </div>
              ...
              <div className="flex justify-between mt-8">
                <button
                  onClick={() => setRecommendationsPage((prev) => Math.max(prev - 1, 1))}
                  disabled={recommendationsPage === 1}
                  className="px-2 py-1 bg-blue-500 text-white rounded-full hover:bg-blue-600 disabled:bg-gray-300"
                >
                  <FaChevronLeft />
                </button>
                <button
  ...
  <p className="text-center">Loading...</p>
    ...
    .length > 0 ? (
    searchResult
      .filter((result) => result._id !== user?._id) // Apply filter again
      .map((result) => (
        <div key={result._id} className="flex items-center mb-4">
          ...
          {user?.connections.includes(result._id) ? (
            <button
              onClick={() => handleMessageButtonClick(result as Connections)}
              className="ml-auto bg-green-500 text-white px-3 py-1 rounded-full text-sm hover:bg-green-600"
            >
              Message
            </button>
          ) : (
            <button
              onClick={() => handleSendRequestForConnection(user?._id as string, result._id)}
              className="ml-auto bg-blue-500 text-white px-3 py-1 rounded-full text-sm hover:bg-blue-600"
            >
              Connect
            </button>
          )}
