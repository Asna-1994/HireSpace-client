...
import { FaUser, FaChevronLeft, FaChevronRight, FaUserPlus, FaUserFriends } from "react-icons/fa";
...
const Connections = () => {
    ...
    const fetchPendingRequests = async (page: number) => {
        try {
            const limit = 8;
            const response = await axiosInstance.get(`/connection-request/to-user/${user?._id}?page=${page}&limit=${limit}`);
            if (response.data.success) {
                const requests = response.data.data.pendingRequests.map((request: any) => request._doc || request);
    ...
    useEffect(() => {
      ...
      const fetchConnections = async (page: number, search: string) => {
        ...
        try {
          const limit = 10;
          const response = await axiosInstance.get(`/connection-request/user/all-connections/${user?._id}`, {
    ...
    const handleRequest = async (requestId: string, action: string) => {
        try {
            const response = await axiosInstance.put(`/connection-request/${requestId}/${action}`);
            if (response.data.success) {
    ...
    const renderPaginationControls = (
        ...
        <div className="flex justify-center items-center mt-4">
            <button
                ...
                className={`p-2 rounded-full ${currentPage === 1 ? "bg-gray-300" : "bg-blue-500 text-white"} transition duration-300`}
            >
                <FaChevronLeft />
            </button>
            <span className="mx-2 text-gray-600">{currentPage} / {totalPages}</span>
            <button
                onClick={() => setPage((prev) => Math.min(prev + 1, totalPages))}
                disabled={currentPage === totalPages}
                className={`p-2 rounded-full ${currentPage === totalPages ? "bg-gray-300" : "bg-blue-500 text-white"} transition duration-300`}
            >
                <FaChevronRight />
            </button>
        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="p-6 border-b border-gray-100">
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">People who want to connect with you</p>
                                </div>
                                    ...
                                    {pendingRequests.length > 0 ? (
                                        <div className="space-y-4">
                                            {pendingRequests.map((req) => (
                                                <div key={req._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <h3 className="font-medium text-gray-900">{req.fromUser.userName}</h3>
                                                            <p className="text-sm text-gray-500">{req.fromUser.tagLine}</p>
                                                        </div>
                                                    </div>
                                ...
                                {pendingRequests.length > 0 && (
                                    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
                                        {renderPaginationControls(pendingPage, pendingTotalPages, setPendingPage)}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">Start making conversations</p>
                                    </div>
                                    <input
                                        type="text"
                                        value={searchTerm}
                                            ...
                                            {connections.map((connection) => (
                                                <div key={connection._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <h3 className="font-medium text-gray-900">{connection.userName}</h3>
                                                            <p className="text-sm text-gray-500">{connection.tagLine}</p>
                                                        </div>
                                                    </div>
                                                    <button
                                ...
                                {connections.length > 0 && (
                                    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
                                        {renderPaginationControls(connectionsPage, connectionsTotalPages, setConnectionsPage)}
                                    </div>
                                )}
                            </div>
                        </div>
