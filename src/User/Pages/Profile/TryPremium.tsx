...
const PricingCard: React.FC<{
  ...
  <motion.div
    ...
    className={`p-6 rounded-xl shadow-lg ${
      selected ? 'bg-gradient-to-r from-purple-400 to-blue-500 text-white' : 'bg-white'
    }`}
  >
    <div className="flex items-center justify-between mb-4">
      <h3 className={`text-2xl font-bold ${selected ? 'text-white' : 'text-gray-800'}`}>
        {plan.planType}
      </h3>
      <FaCrown className={`text-2xl ${selected ? 'text-white' : 'text-blue-500'}`} />
    </div>
    <div className="mb-6">
      <span className="text-3xl font-bold">₹{plan.price}</span>
      <span className={`${selected ? 'text-white' : 'text-gray-600'}`}>/{plan.durationInDays}</span>
    </div>
    <ul className="space-y-3 mb-6">
      {plan.features.map((feature, index) => (
        <li key={index} className="flex items-center">
          <FaCheck className={`mr-2 ${selected ? 'text-white' : 'text-green-500'}`} />
          <span className={selected ? 'text-white' : 'text-gray-600'}>{feature}</span>
        </li>
      ))}
    </ul>
    <button
...
const PremiumPlans: React.FC = () => {
  ...
  <div className="text-center mb-12">
    <h2 className="text-4xl font-extrabold text-gray-900 mb-4">Upgrade to Premium</h2>
    <p className="text-lg text-gray-600 max-w-2xl mx-auto">
      Unlock exclusive features and accelerate your success with our premium plans.
    </p>
  </div>
  ...
  <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-8 max-w-6xl mx-auto mb-12">
    ...
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg shadow-lg p-8 border-2 border-blue-400 text-center">
      ...
      <p className="text-gray-600 mb-6">Your current plan with essential features for beginners.</p>
      ...
      <ul className="text-left text-gray-700 mb-6 space-y-3">
        ...
        <li className="flex items-center">
          <span className="text-green-500 mr-2">✔</span> Limited applications
        </li>
        <li className="flex items-center">
