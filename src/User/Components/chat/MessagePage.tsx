...
//       const handleUnreadCounts = ({ counts }: { counts: { [roomId: string]: number } }) => {
//         setChats(prevChats =>
//           prevChats.map(chat => ({
//             ...chat,
//             unreadCount: counts[chat.roomId] || 0,
...
//       const handleRecentChats = (data: { chats: RecentChat[] }) => {
//         setChats(data.chats);
//         socket.emit('getUnreadCount', { userId: user._id });
//       };
...
//       socket.on('recentChats', handleRecentChats);
...
//         socket.off('recentChats', handleRecentChats);
...
//               <li
//                 key={chat.roomId}
//                 className="flex items-center p-4 bg-white shadow-md rounded-lg hover:shadow-lg transition-shadow cursor-pointer"
